...
import styles from "./ChatOnline.module.css";
import { getIdUser, getTokenFromLocalStorage } from "../../utils/tokenUtils";
...
const ChatOnline = () => {
  ...
  const [receiverId, setReceiverId] = useState(44);
  ...
  const handleSendMessage = useCallback(() => {
    if (messageInput.trim() !== "") {
      const roomId = `${IdUser}-${receiverId}`;
      const message = {
        content: messageInput,
        senderId: IdUser,
        receiverId: 44, // Người nhận cố định
      };
                  ...
                  >
                    {messages.map((message, index) => (
                      <div
                        key={index}
                        className={`media media-chat ${
                          message.content.senderId === IdUser
                            ? "media-chat-reverse"
                            : ""
                        }`}
                      >
